...
# Tutorial 1
In this tutorial, we will explore several data wrangling and descriptive statistics tasks using the dataset named educ_inc_age_data.csv. This dataset contains information about individuals' age, income, and education level.
...
- R has many integrated functions for many different purposes. They all have the same basic format: function_name(argument1 = value 1, argument2 = value 2, …). You can display the arguments for a function by typing ‘?function_name’ in the console. Not all arguments require values – for some, R has a default value that it will use if you don’t specify it.
...
- Packagesneed to be installed once (install.packages(“package_name”)), and loaded every time you open R (library(package_name)).
...
# Assign 'Middle-aged' to individuals aged 30 up to, but not including, 65.
educ_inc_age_data$age_group[educ_inc_age_data$age >= 30 & educ_inc_age_data$age < 65] <- "Middle-aged"

# Assign 'Elderly' to individuals aged 65 and above
...
# Mode
# Mode function is not readily available in base R. You can however create our own function (though maybe this is a bit advanced, so we try it later in the course), or use a function from a package, such as DescTools.
